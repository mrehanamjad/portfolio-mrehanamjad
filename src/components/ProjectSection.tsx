...
function ProjectSection() {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(
      ".projects-subtitle-word",
      { y: 60, opacity: 0, rotationX: isMobile ? 0 : 90 },
      {
        y: 0,
        opacity: 1,
        rotationX: 0,
    ...
    const mainTl = gsap.timeline({
      scrollTrigger: {
        trigger: cardsRef.current,
        start: "top top",
        end: `+=${cards.length * 60}%`, // shorter distance
        scrub: 0.5,
        pin: true,
        anticipatePin: 1,
      },
      ...
      <Container>
        <div id="project-section-main-container" className="w-full overflow-hidden">
          ...
          <div className="pt-20 pb-16 text-center">
            ...
            <h3 className="text-center text-6xl md:text-8xl uppercase font-bold">
              {["Some", "Of", "My", "Recent", "Work"].map((word, i) => (
                <span
                  key={i}
                  className="projects-subtitle-word bg-gradient-to-t from-black/40 via-white to-white inline-block text-transparent bg-clip-text mr-2"
                >
                  {word}
                </span>
              ))}
          ...
          >
            <Link
              ...
              target="_blank"
              className="flex justify-center items-end gap-1 cursor-pointer group hover:text-green-500"
            >
              <span className="relative text-2xl md:text-6xl font-bold italic transition-colors duration-300 after:content-[''] after:absolute after:left-0 after:-bottom-2 after:w-0 after:h-[4px] after:bg-green-700 after:transition-all after:duration-300 group-hover:after:w-full flex gap-2">
                View All Projects{" "}
                <LuArrowUpRight className="text-2xl md:text-7xl group-hover:rotate-12 transition-transform" />
              </span>
            </Link>
            ...
            {cards.map((card, index) => (
              ...
              >
                {/* Left */}
                <div
                  className={`md:pr-1 max-md:px-1 md:py-4 flex flex-col md:w-[35%] h-full ${
                    index % 2 === 0 ? "order-1" : "order-2 md:pl-3"
                  } max-md:order-2 max-md:justify-between`}
                >
                  <div className="w-full h-full flex flex-col gap-4">
                    <h4 className="font-bold text-3xl">{card.title}</h4>
                    <p className="md:text-lg">{card.description}</p>
                  ...
                  <div className="flex justify-end items-center px-2 gap-2 max-md:justify-between max-md:px-4 max-md:pb-4 pt-2">
                    <Link
                      href={card.github}
                      target="_blank"
                      className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 transition-all duration-200 group text-sm sm:text-base"
                      title="View Code"
                    >
                      <FaGithub className="w-4 h-4 sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform" />
                      <span className="inline md:hidden">Code</span>
                ...
                {/* Right */}
                <div
                  className={`relative bg-black/20 w-full md:w-[65%] h-[40%] md:h-full rounded-xl max-md:order-1 md:flex-1 ${
                ...
                >
                  ...
                  <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
                    <span
                      className={`px-3 py-1.5 text-xs sm:text-sm font-medium ${card.color} backdrop-blur-sm rounded-full border border-white/20 text-white antialiased`}
                    >
                      {card.category}
                    </span>
                  </div>
